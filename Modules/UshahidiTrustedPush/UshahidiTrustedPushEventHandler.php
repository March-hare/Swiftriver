...
class UshahidiTrustedPushEventHandler implements \Swiftriver\Core\EventDistribution\IEventHandler {
    /**
    ...
    public function HandleEvent($event, $configuration, $logger) {
        $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [Method invoked]", \PEAR_LOG_DEBUG);
        ...
        if(!key_exists($this->Name(), $config)) {
            $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [The Ushahidi Event Handler was called but no configuration exists for this module]", \PEAR_LOG_ERR);
            $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [Method finished]", \PEAR_LOG_DEBUG);
            return;
        }
        ...
        foreach($this->ReturnRequiredParameters() as $requiredParam) {
            if(!key_exists($requiredParam->name, $config)) {
                $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [The Ushahidi Event Handler was called but all the required configuration properties could not be loaded]", \PEAR_LOG_ERR);
                $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [Method finished]", \PEAR_LOG_DEBUG);
                return;
            }
        }
        ...
        $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [Pushing trusted content item to Ushahidi]", \PEAR_LOG_DEBUG);
        ...
        foreach($content as $item) {
            ...
            if($item->source->score > 90) {
                ...
                $toUshahidiParser = new \Swiftriver\UshahidiTrustedPush\ContentToUshahidiAPIParser();
                ...
                $service = new \Swiftriver\UshahidiTrustedPush\ServiceInterface();
                ...
                try {
                    ...
                    $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [An exception was thrown]", \PEAR_LOG_ERR);
                    $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [$e]", \PEAR_LOG_ERR);
                ...
                $logger->log("Swiftriver::EventHandlers::UshahidiTrustedPushEventHandler::HandleEvent [Content item not pushed, score < 9, score is:".$item->source->score."]", \PEAR_LOG_DEBUG);
            }
        }
